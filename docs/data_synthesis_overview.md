...
**核心思想**：在参数邻域采样新参数，运行 MODFLOW 生成新时序
...
#### 优点
- ✅ 完全符合物理规律（保持参数-时序映射的确定性）
- ✅ 提高模型精度 2-3 倍
- ✅ 增加参数空间覆盖 50%
- ✅ 无矛盾训练数据

**注**：V1 的时序扰动方法（Scaling/Offset）已废弃，因为它破坏了物理一致性（相同参数 → 不同输出）。
...
### `augmenters/parameter_augmentation.py`
- **职责**：参数空间采样增强（V2）
- **核心函数**：
  - `perturb_params()` — 扰动参数
  - `augment_with_parameter_sampling()` — 完整增强流程

### `generators/modflow_generator_with_params.py`
- **职责**：从指定参数生成样本
- **核心函数**：
  - `generate_sample_from_params()` — 从指定参数生成单个样本
  - `generate_batch_from_params()` — 批量生成
...
### `pipeline/modflow_pipeline_v2.py`
- **职责**：端到端流程编排（使用参数空间采样增强）
- **核心函数**：
  - `run_pipeline()` — 执行完整管线（生成→过滤→参数空间增强→存储）
...
### 1. 配置文件
编辑 `configs/data_synthesis/modflow_v2.yaml`：
```yaml
